function PollClass(){
	...
	this.__getHtml = function(mode){
		...
		if(this.properties.isHiddenTitle==false){
			html = html +'  <div class="headerPoll ui-widget-header ui-corner-all ui-widget">Titulo</div>\n';
		}
	...
	this.finish = function(){
		if(this.properties.onFinish != undefined && this.properties.onFinish != null && this.properties.onFinish != ""){
			var ret;
			eval("ret = this.properties.controller."+this.properties.onFinish+"(this.properties.pages)");
			return ret;
		...
		if(question.type == "text"){
			...
			for(var i=0; i<question.list.length; i++){
				...
				if(typeChek == 'other'){
					...
					html = html + "<input  type='radio' subtype='radio' "+checked+" index='"+id+"' subid='"+i+"' idtext='"+idText +"' typeChek='other' name='"+question.name+"' id='"+radioId+id+"' type='radio' value=''/>"+question.list[i].caption+"<input type'text' value='"+value+"' id='"+idText+"' onkeyup ="+onkeyup +"><br/>";
				}else{
					if(question.value != undefined && question.value == question.list[i].value){
						checked = "checked";
					}
					html = html + "<input type='radio' subtype='radio' "+checked+" index='"+id+"' subid='"+i+"' name='"+question.name+"' id='"+radioId+id+"' type='radio' value='"+ question.list[i].value+"'/>"+question.list[i].caption+"<br/>";
				}
			}
			...
			html = html + "<input  type='checkbox' "+checked+" index='"+id+"' name='"+question.name+" id='"+checkboxId+"'/>"+question.title;
			...
			for(var i=0; i<rows.length; i++){
				...
				for(var e=0; e<cols.length; e++){
					...
					html = html + "<td align='center'><input  type='radio' subtype='grid' "+checked
			...
			html = "<div><b class='"+question.cssClass+"'>"+question.title +"</b></div>";

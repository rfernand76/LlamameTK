...
function DynatreeClass(){
	...
	this.create = function(padre, mode){
		...
		$("#"+this.properties.id).dynatree({
            onActivate: function(node) {
				if(me.properties.onActivate && me.properties.controller){
					eval("me.properties.controller."+me.properties.onActivate+"(node)");
				}
            }
			,children:me.properties.data
		});
	...
	this.setEvents = function(){
		...
		if(this.properties.events){
			var el = $("#"+this.properties.id);
			var me = this;
			
			for(var i=0; i<this.properties.events.length; i++){
				
				var name  = this.properties.events[i].name;
				var event = this.properties.events[i].event;
				var me = this;
				
				
				el.on(name , function() {
					var tree = $("#"+me.properties.id);
					var node = tree.dynatree("getActiveNode");
					if (node) {
						eval("me.properties.controller."+event+"(node)");
					}
				});
			}
		}
	...
	this.resize = function(heightResize100, widthResize100){
		...
		this.height = heightResize100;
		this.width = widthResize100;
	}
...
,{"subclass":"DivClass","parent":"panelTab_1","height":"151","width":"249","top":"42","left":"16",
	...
	this.setModel = function(model){
		...
		var objTree = $("#"+this.properties.id);
		var rootNode = objTree.dynatree("getRoot");
		rootNode.removeChildren();
		var childNode = rootNode.addChild(obj);	
	}	
}
